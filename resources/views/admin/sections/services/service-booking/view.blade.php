...
@section('breadcrumb')
    @include('admin.components.breadcrumb',['breadcrumbs' => [
        ...
        [
            'name'  => __("Bookings"),
            'url'   => setRoute("admin.service.booking.index"),
        ]
        ...
        <div class="row mb-30-none">
            <div class="col-lg-4 mb-30">
                <div class="booking-area">
                    <h4 class="title"><i class="fas fa-water text--base me-2"></i>{{ __("Service Information") }}</h4>
                    <div class="thumb">
                        <img src="{{ asset('/public/images/services/'.$booking->service->image) }}" alt="profile">
                    </div>
                    <div class="content">
                        <div class="list-wrapper">
                            <ul class="list">
                                <li>{{ __("name") }}:<span>{{ $booking->service->name ?? "" }}</span></li>
{{--                                <li>{{ __("Number") }}:<span>{{ $booking->service->number_bottels ?? "" }}</span></li>--}}
                                <li>{{ __("size") }}:<span>{{ $booking->service->size ?? "" }}</span></li>
{{--                                <li>{{ __("Rate") }}:<span>{{ get_amount($booking->service->price) ?? "" }}{{ __("/KM") }} {{ $default_currency->code }}</span></li>--}}
                                <li>{{ __("Rate") }}:<span>{{ get_amount($booking->service->price) ?? "" }} {{ $default_currency->code }}</span></li>
                                <li>{{ __("Service Type") }}:<span>{{ $booking->service->type->name ?? "" }}</span></li>
                            </ul>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="content pt-0">
                        <div class="list-wrapper">
                            <ul class="list">
                                ...
                                <li>{{ __("Discount") }}:<span>{{ ($booking->discount_amount == 0) ? __("N/A") : get_amount($booking->discount_amount,$default_currency->code,2) }}</span></li>
                                <li>{{ __("Final Price") }}:<span>{{ ($booking->final_price == 0) ? __("N/A") : get_amount($booking->final_price,$default_currency->code,2) }}</span></li>
                            </ul>
                        </div>
                    </div>
                </div>
