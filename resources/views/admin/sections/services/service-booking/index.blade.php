        ...
        <div class="table-wrapper">
            ...
            <div class="table-responsive">
                <table class="custom-table">
                    ...
                    <tbody>
                    @forelse ($service_bookings ?? [] as $key => $item)
                        <tr data-item="{{ json_encode($item->only(['id','status'])) }}">
                            <td>{{ $key + $service_bookings->firstItem() }}</td>
                            ...
                            <td>
                                ...
                                @include('admin.components.link.custom',[
                                    ...
                                    'permission'    => "admin.service.booking.status",
                                ])
                            </td>
{{--                            <td>{{ get_amount($item->service->price) ?? " " }}{{ __("/KM") }} {{ $default_currency->code }}</td>--}}
                            <td>{{ get_amount($item->service->price) ?? " " }} {{ $default_currency->code }}</td>
                            <td>{{ get_amount($item->discount_amount) ?? " " }} {{ $default_currency->code }}</td>
                            <td>{{ get_amount($item->final_price) ?? " " }} {{ $default_currency->code }}</td>
                            ...
                            <td>
                                @include('admin.components.link.custom',[
                                    ...
                                    'text'          => __("Add Fare"),
                                    'permission'    => "admin.service.booking.fare",
                                ])
                                @include('admin.components.link.custom',[
                                    'href'          => "#send-reply",
                                    'class'         => "btn btn--base reply-button modal-btn",
                                    'icon'          => "las la-envelope-open-text",
                                    'permission'    => "admin.service.booking.messages.reply",
                                ])
                                <a href="{{ setRoute('admin.service.booking.details',$item->slug)}}" class="btn btn--base btn--primary"><i class="las la-info-circle"></i></a>
                            </td>
                        </tr>
                    @empty
        ...
        {{ get_paginate($service_bookings) }}
    </div>
    {{-- Add Fare Modal --}}
    @if (admin_permission_by_name("admin.service.booking.fare"))
        <div id="fare-add" class="mfp-hide large">
            <div class="modal-data">
                ...
                <div class="modal-form-data">
                    <form class="card-form" action="{{ setRoute('admin.service.booking.fare') }}" method="POST">
                        @csrf
    ...
    @if (admin_permission_by_name("admin.booking.messages.reply"))
        <div id="send-reply" class="mfp-hide large">
            <div class="modal-data">
                ...
                <div class="modal-form-data">
                    <form class="card-form" action="{{ setRoute('admin.service.booking.messages.reply') }}" method="POST">
                        @csrf
                        <input type="hidden" name="target" value="{{ old('target') }}">
                        <div class="row mb-10-none">
                            <div class="col-xl-12 col-lg-12 form-group">
    ...
    @if (admin_permission_by_name("admin.booking.state"))
        <div id="status-change" class="mfp-hide large">
            <div class="modal-data">
                ...
                <div class="modal-form-data">
                    <form class="card-form" action="{{ setRoute('admin.service.booking.status') }}" method="POST">
                        @csrf
                        <input type="hidden" name="target" value="{{ old('target') }}">
                        <div class="row mb-10-none">
                            <div class="col-xl-12 col-lg-12 form-group">
